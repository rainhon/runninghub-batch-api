...
import type { AppMissionDetail, AppMissionItem } from '../types';
...
export default function AppTaskDetailPage() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const missionId = parseInt(id || '0');

  const [mission, setMission] = useState<AppMissionDetail | null>(null);
  const [items, setItems] = useState<AppMissionItem[]>([]);
  ...
  const loadDetail = async (showRefreshing = false) => {
    try {
      ...
      const [missionResult, itemsResult] = await Promise.all([
        api.getAppMissionDetail(missionId),
        api.getAppMissionItems(missionId),
      ]);
  ...
  // 下载单个结果
  const handleDownloadItem = (item: AppMissionItem) => {
    if (!item.result_url) return;

    const link = document.createElement('a');
    link.href = item.result_url;
    link.download = `result_${item.item_index}`;
  ...
  // 渲染子任务卡片
  const renderItemCard = (item: AppMissionItem) => {
